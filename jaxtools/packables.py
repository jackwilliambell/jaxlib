...
from basetypes import PropertySheet, PackedState
...
safe version of the Packable object to use."""
...
with the Packable object's state design and interface."""
	...
	def pack(self, properties, hints):
		"""Implementations must load the passed properties with 
...
packed so long as it supports the same state format.
...
be invoked in case it supports that State ID."""

	def makePackable(stateId, properties, hints):
		"""TODO."""
		raise NotImplementedError("Abstract method, must be implemented if subclass supports it..")
...
object."""
	stateId = overrideStateId ? overrideStateId : packable.getStateId()
...
the State ID."""
	stateId = overrideStateId ? overrideStateId : state.stateId
